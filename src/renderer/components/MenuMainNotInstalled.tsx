...
export default function MenuMainNotInstalled() {
  const [currentFilter, workspaces] = useRecoilValue(
    waitForAll([
      SettingCurrentFilterSelector,
      WorkspaceListByFilterSelector('not_installed'),
    ])
  )
  ...
  return (
    <MenuMainDefault
      active={currentFilter === 'not_installed'}
      icon="Cloud"
      name="Not installed"
      count={workspaces.length}
      callback={() =>
        updateSettings({ currentFilter: 'not_installed', currentFolder: null })
      }
    />
  )
}
